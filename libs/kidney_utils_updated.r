...
plot_shuffled_features <- function(input_df, input_norm, feature, pt_size,pct =NA,
...
plot_cna_umap <- function(meta, fdr_thresh, full_meta, hUMAP_col = FALSE) {
    if (hUMAPcol == TRUE) {
        meta <- meta %>%  mutate(huwotUMAP1 = hUMAP1,
                                 huwotUMAP2 = hUMAP2)
        full_meta <- full_meta %% mutate(huwotUMAP1 = hUMAP1,
                                         huwotUMAP2 = hUMAP2)
    }
    tmp_meta <- meta %>% mutate(ifelse(abs(meta$ncorr) > fdr_thresh, ncorr, NA))
    
    tmp_meta <- cbind(tmp_meta, umap)
    cna_umap <- ggplot() + 
          geom_point(data = full_meta,
