...
res_1 <- mclapply(keep_genes[1:10], FUN = gene_ncorr_perm, mc.cores = 5)
Sys.time()

res <- mclapply(keep_genes, FUN = gene_ncorr_perm, mc.cores = 10)

out <- data.frame(gene = keep_gene, ppval = do.call(rbind, res))
