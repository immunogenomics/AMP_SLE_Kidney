...
exprs_norm <- readRDS(paste("/data/srlab2/jmears/jupyter/SLE_Phase2_Kidney_Analysis/nuc-seq-output/seurat_output/",
...
for (i in 1:length(markers)) {
  ...
  gene <- markers[i]
    ...
    ind <- ggplot(meta) +
      stat_summary_hex(aes(x = uwotUMAP1, y = uwotUMAP2, z = gene
#                          fill = cut(..value..,
#                                     c(0, 0.1, 0.5, 1, 3, Inf))
                        ),
                     fun = mean,
...
#     scale_fill_viridis(option = "viridis", end = .9) +
    ...
    pdf(paste("/data/srlab2/jmears/jupyter/SLE_Phase2_Kidney_Analysis/nuc-seq-output/expression_plots/",
        gene,
        "_exp_allsamples.pdf",
        sep = ""),
        height = 4,
