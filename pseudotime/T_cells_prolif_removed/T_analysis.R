...
sds <- readRDS("T_destiny_slingshot.Rds")
...
plot_df$final_annotation <- meta$annotationNumber[match(rownames(plot_df), meta$barcode)]
...
for (i in seq_along(lineages)) {
 ...
 # Define a nice color palette
    ...
    pdf(paste0("slingshot/lineage_", i, "/cellPlots", i, "_pseudotime.pdf"), width = 9, height = 7)
        print(
        ggplot() +
            ...
            geom_label_repel(
                ...
                label.padding = unit(0.15, "lines"), size = 4, fontface = "bold", show.legend = FALSE
            ) + 
            theme_minimal(base_size = 18) +
