...
sds <- readRDS("pt_outputs/CD8_destiny_slingshot.Rds")
...
meta_4 <- read.csv("CD8_inputs/CD8_T_cells_meta.csv", row.names = 1)
...
pdf("figures/CD8_destiny_TSM.pdf", width = 8, height = 6)
...
for (i in seq_along(lineages)) {
 ...
 # Define a nice color palette
    ...
    pdf(paste0("slingshot_CD8/lineage_", i, "/cellPlots", i, "_pseudotime.pdf"), width = 9, height = 7)
        print(
        ggplot() +
    ...
    write.csv(geneStats, paste0("slingshot_CD8/lineage_", i, "/correlatedGenes.csv"), row.names = FALSE)
    ...
    dir.create(paste0("slingshot_CD8/lineage_", i, "/topGenes/"))
    ...
    for (gene in topGenes) {
        ...
        pdf(paste0("slingshot_CD8/lineage_", i, "/topGenes/", gene, "_expression.pdf"), width = 9, height = 7)
            print(ggplot() +
