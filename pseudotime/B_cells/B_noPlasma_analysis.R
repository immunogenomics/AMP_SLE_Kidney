...
sds <- readRDS("B_destiny_slingshot.Rds")

bloodNorm <- readRDS("/data/srlab/ssg34/SLE_kidney_v2/sugiarto_scripts/finalObjects_v3/b_pbmc_norm.Rds")
tissueNorm <- readRDS("/data/srlab/ssg34/SLE_kidney_v2/sugiarto_scripts/finalObjects_v3/b_tissue_norm.Rds")
...
counters <- read.csv("/data/srlab/ssg34/SLE_kidney_v2/sugiarto_scripts/harmonyIntegration_finalFinal/B/harmony_0/pbmcVsTissue.csv")
...
# Step 1: Keep only row.names(dPCs) that exist in allNorm columns
common_barcodes <- row.names(dPCs)[row.names(dPCs) %in% colnames(allNorm)]

# Step 2: Subset plot_df
plot_df <- plot_df[common_barcodes, , drop = FALSE]  # keep it as a data.frame

# Step 3: Reorder allNorm columns to match plot_df
allNorm <- allNorm[, common_barcodes]
...
for (i in seq_along(lineages)) {
    ...
    dir.create(paste0("slingshot/lineage_", i))

    # Extract pseudotime
    pt <- sds@assays@data@listData$pseudotime[, i]
    pt <- pt[common_barcodes]
 ...
 # Define a nice color palette
    ...
    pdf(paste0("slingshot/lineage_", i, "/cellPlots", i, "_pseudotime.pdf"), width = 6, height = 5)
        print(
        ggplot() +
            ...
            geom_label_repel(
                ...
                label.padding = unit(0.15, "lines"), size = 5, fontface = "bold", show.legend = FALSE
            ) + 
            theme_minimal(base_size = 18) +
    ...
    # write.csv(geneStats, paste0("slingshot/lineage_", i, "/correlatedGenes.csv"), row.names = FALSE)
    ...
    # dir.create(paste0("slingshot/lineage_", i, "/topGenes/"))
    ...
    #     pdf(paste0("slingshot/lineage_", i, "/topGenes/", gene, "_expression.pdf"), width = 9, height = 7)
    #         print(ggplot() +
...
ptExpr <- na.omit(data.frame(
    Pseudotime = pt, 
    ITGAX = itgax,
    DC1 = plot_df$DC1,
    DC2 = plot_df$DC2))

print(cor.test(ptExpr$Pseudotime, ptExpr$ITGAX))

pdf("itgax_pseudotimeExpr.pdf", width = 6, height = 5)
    ggplot(ptExpr, aes(x = DC1, y = DC2, color = ITGAX)) + 
