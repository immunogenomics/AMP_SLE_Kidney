...
meta <- readRDS("../finalObjects_v2/myeloid_tissue_meta.Rds") %>% select(cell, sample, dataset, final_annotation) %>% filter(final_annotation == "M5. GPNMBhigh NUPR1high Macrophage")
...
ggsave("proliferation_scores_by_chronicity.pdf", plot = p, width = 8, height = 6)
...
ggsave("proliferation_scores_by_activity.pdf", plot = p, width = 8, height = 6)
