...
## Read in metadata
	raw_meta <- readRDS(paste("../cell-seq-output/raw_metadata/",
                            samples[i],
                            "_raw_meta.rds",
                            sep = "")
			)
...
# extra columns to add to QC_df
	Num.droplets.500.genes[i] <- nrow(raw_meta[raw_meta$nGene > 500,])
	Num.droplets.1000.genes[i] <- nrow(raw_meta[raw_meta$nGene > 1000,])
...
saveRDS(QC_df, "/data/srlab2/jmears/jupyter/SLE_Phase2_Kidney_Analysis/cell-seq-output/QC_output/QC_output_all.rds")
