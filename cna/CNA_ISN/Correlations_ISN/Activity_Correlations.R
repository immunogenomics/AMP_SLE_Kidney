...
plotCellType <- function(cellType, xlim, ylim) {
    i_corr <- read.csv(paste0("../cna_results/", cellType, "/Final_ISN_[V]_SiteFBChron_ncorr.csv"), header = FALSE)
    i_fdrs <- read.csv(paste0("../cna_results/", cellType, "/Final_ISN_[V]_SiteFBChron_fdrs.csv"), header = FALSE)
    ...
    a_corr <- read.csv(paste0("../../CNA_activityRerun/cna_results/", cellType, "/Final_Activity_SiteFBChron_ncorr.csv"), header = FALSE)
    a_fdrs <- read.csv(paste0("../../CNA_activityRerun/cna_results/", cellType, "/Final_Activity_SiteFBChron_fdrs.csv"), header = FALSE)
    a_meta <- read.csv(paste0("/data/srlab/ssg34/SLE_kidney_v2/data/cna_new/", cellType, "/activity/sc_meta.csv"))
    ...
    plot_df <- data.frame(ISN = i_corr$V1[i_corr$cell %in% cellsToKeep], Activity = a_corr$V1[a_corr$cell %in% cellsToKeep])
    ...
    plot_df <- plot_df %>% mutate(i_status = case_when(
        ISN > i_threshold ~ "Expanded",
        ISN < -1 * i_threshold ~ "Depleted",
        TRUE ~ "Unchanged"
    ))
    ...
    p <- ggplot(data = plot_df, aes(x = Activity, y = ISN, color = color)) + 
        ...
        annotate("text", x = Inf, y = -Inf, label = paste0("R = ", round(cor(plot_df$Activity, plot_df$ISN), 3)), hjust = 1.1, vjust = -0.5, size = 5) + 
