...
plotType <- function(coVars) {
  meta <- read.csv(paste0('/data/srlab/ssg34/SLE_kidney_v2/data/cna_new/', name, '/ISN/sc_meta.csv'))
  umap <- read.csv(paste0('/data/srlab/ssg34/SLE_kidney_v2/data/cna_new/', name, '/ISN/sc_umap.csv'))
  ncorr <- read.csv(paste0("cna_results/", name, "/Final_ISN_[V]_", coVars, "_ncorr.csv"), header = FALSE)
  fdrs <- read.csv(paste0("cna_results/", name, "/Final_ISN_[V]_", coVars, "_fdrs.csv"), header = FALSE)
  ...
  height = max(round(length(unique(meta$final_annotation_num)) / 3.7), 4)
  width = 4
...
plotType("SiteFBChron_Race")
plotType("SiteFBChron_Activity")
