...
python activityCNA.py $name tissue $meta
EOF
done
...
for name in ${types[@]}; do
  ...
  mkdir -p cna_results/${name}_pbmc
  echo $name
  cat << EOF | sbatch 
#!/bin/bash
#SBATCH -J ${name}_CNA
#SBATCH -D ${folder}
#SBATCH -o ${folder}/log/${name}_pbmc_CNA.out.txt
#SBATCH -e ${folder}/log/${name}_pbmc_CNA.err.txt
#SBATCH -p long
#SBATCH --mem=8G
#SBATCH -c 8
#SBATCH --mail-type=end
...
python activityCNA.py $name pbmc $meta
EOF
done