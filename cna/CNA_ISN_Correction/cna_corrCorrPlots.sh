...
for name in ${types[@]}; do
    ...
    for file in cna_results/${name}_tissue/Final_Activity*_ncorr.csv; do
...
#SBATCH -J ${name}_CNA
#SBATCH -D ${folder}
#SBATCH -o ${folder}/log/${name}_tissue_$(basename $file)_corrCorr.out.txt
#SBATCH -e ${folder}/log/${name}_tissue_$(basename $file)_corrCorr.err.txt
#SBATCH -p bigmem
#SBATCH --mem=16G
#SBATCH -c 8
...
Rscript corrCorrPlots.R $name tissue $file $meta
EOF
  done
done
...
for name in ${types[@]}; do
    ...
    for file in cna_results/${name}_pbmc/Final_Activity*_ncorr.csv; do
...
#SBATCH -J ${name}_CNA
#SBATCH -D ${folder}
#SBATCH -o ${folder}/log/${name}_pbmc_$(basename $file)_corrCorr.out.txt
#SBATCH -e ${folder}/log/${name}_pbmc_$(basename $file)_corrCorr.err.txt
#SBATCH -p bigmem
#SBATCH --mem=16G
#SBATCH -c 8
#SBATCH --mail-type=end
...
Rscript corrCorrPlots.R $name pbmc $file $meta
EOF
    done
done