...
testCorr <- function(cellType) {
    gex <- norm[, colnames(norm) %in% cna_meta$cell & colnames(norm) %in% meta$cell[meta$final_annotation == cellType]]
    name <- gsub(" ", "_", clean_strings(unique(cellType)))
    ...
    correlations <- correlations[!(is.na(correlations$corr)), ] 
    correlations_sorted <- correlations[order(-correlations$corr), ]
    write.csv(correlations_sorted, paste0("geneCorrs_csvs/", name, ".csv"))
}

dir.create("geneCorrs_csvs")
lapply(unique(meta$final_annotation), testCorr)