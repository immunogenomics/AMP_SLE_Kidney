...
plotType <- function(coVars) {
  meta <- read.csv(paste0('ptChronicities/', name, '/sc_meta.csv'))
  umap <- read.csv(paste0('ptChronicities/', name, '/sc_umap.csv'))
  ...
  globalp = as.numeric(pValues[pValues$type == coVars, ]$pVal)
  pheno = 'LN'  
...
plotType("Final_Chronicity_none")
plotType("Final_Chronicity_PT")
plotType("injured_pt_prop_none")
plotType("injured_pt_prop_chronicity")