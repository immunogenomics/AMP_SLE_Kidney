#@Script: B Cell Case Control MASC PBMC
#@Author: Sid Gurajala
#@Date:04/19/2024
...
model_df <- meta %>% select(Unified_Visit, Type, nCount_RNA, percent.mt, RNA_snn_res.0.25)
model_df <- model_df %>% 
                mutate(
                       nCount_RNA = log(nCount_RNA))

res <- MASC.me(model_df, factor(model_df$RNA_snn_res.0.25),
                contrast = "Type",
...
saveRDS(res, '/data/srlab/ssg34/SLE_kidney_v2/data/masc/bcell_pbmc/case_control.rds')