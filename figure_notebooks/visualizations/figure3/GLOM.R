...
umap <- read.csv('/data/srlab/ssg34/SLE_kidney_v2/data/cna_new/GLOM/case_control/sc_umap.csv')
ncorr <- read.csv("/data/srlab/ssg34/SLE_kidney_v2/data/cna_new/GLOM/case_control/sc_ncorr.csv", header = FALSE)
fdrs <- read.csv("/data/srlab/ssg34/SLE_kidney_v2/data/cna_new/GLOM/case_control/sc_fdrs.csv", header = FALSE)
...
subset <- c('GLOM0. BCAMhigh AKAP12high PEC', 
...
cowplot::save_plot(paste0(figdir, 'GLOM_UMAP_legend.pdf'), plot = umap_legend,  
       base_height = 1, base_width = 3.5, dpi = 500)
ggsave(paste0(figdir, 'GLOM_UMAP.pdf'), plot = cna_umap,  
...
ggsave(paste0(figdir, 'GLOM_UMAP_sorted.pdf'), plot = cna_umap,  
       height = 4, width = 3.5, dpi = 500, device = cairo_pdf)
...
pdf(paste0(figdir, "GLOM_full.pdf"), width = width, height = height)
    print(violin_plot)
dev.off()
...
ggsave(paste0(figdir, 'GLOM_sc_vlnPlot_medians.png'), plot = outplot,  
       height = height, width = width, dpi = 500)
ggsave(paste0(figdir, 'GLOM_sc_vlnPlot_medians.pdf'), plot = outplot,  
