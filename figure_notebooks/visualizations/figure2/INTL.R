...
meta <- readRDS("../reorderedClusters/intl_meta_reOrdered.Rds")
...
meta <- meta %>%
  mutate(new_cluster_number = str_extract(as.character(final_annotation), "^[^.]+"),  # Extract everything before the period
         new_cluster_number = ifelse(str_starts(new_cluster_number, "EC"), substr(new_cluster_number, 3, nchar(new_cluster_number)), substr(new_cluster_number, 4, nchar(new_cluster_number))))
...
cowplot::save_plot("INTL_Tissue_legend.pdf",
       legend,
       base_height = 10,
       base_width = 10)

pdf("INTL_Tissue.pdf", width = 5, height = 5)
    print(p)
dev.off()