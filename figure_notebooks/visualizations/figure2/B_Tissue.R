...
meta <- readRDS("../reorderedClusters/b_tissue_meta_reOrdered.Rds")
...
# New clustering
p <- ggplot(meta, aes(x = hUMAP1, y = hUMAP2, color = final_annotation)) +
...
cowplot::save_plot("B_Tissue_legend.pdf",
       legend,
       base_height = 10,
       base_width = 10)

pdf("B_Tissue.pdf", width = 5, height = 5)
    print(p)
dev.off()