...
ncorr <- read.csv("../CNA_chronicityCorrections/cna_results/myeloid/Final_Chronicity_SiteFirstBiopsy_ncorr.csv", header = FALSE)
fdrs <- read.csv("../CNA_chronicityCorrections/cna_results/myeloid/Final_Chronicity_SiteFirstBiopsy_fdrs.csv", header = FALSE)
...
vlnPlot <- ggplot(plotMeta, aes(x = type, y = ncorr)) +
                    ...
                    labs( y= "Chronicity CNA Correlation", x = "", title = "") +
...
pdf("SAMs_Chronicity_vlns.pdf", width = 5, height = 5)
    print(vlnPlot)
dev.off()